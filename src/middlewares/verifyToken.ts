...
export const verifyAccessToken = async(req: Request, res: Response, next: NextFunction) => {
    ...
    if(!authHeader) throw new ApiError('Missing authentication token', 401, 'verifyToken.file')
...
export const verifyRefreshToken = async(req: Request, res: Response) => {
    ...
    if (!refreshToken) throw new ApiError('Refresh token is missing from cookies', 401);
